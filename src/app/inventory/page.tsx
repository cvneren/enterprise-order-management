...
export default function InventoryPage() {
  ...
  const filteredRoasts = useMemo(() => {
    return INITIAL_ROASTS.filter((roast) => {
      const matchesSearch = roast.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
                           roast.type.toLowerCase().includes(searchQuery.toLowerCase());
      const matchesStatus = statusFilter === "All" || roast.status === statusFilter;
    ...
    <div className="py-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-surface border border-border rounded-md text-sm focus:ring-1 focus:ring-primary focus:border-primary font-sans text-on-surface"
            />
          </div>
          <div className="relative w-full sm:w-44">
            <Filter className="w-4 h-4 text-on-surface-variant absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
              className="w-full pl-9 pr-4 py-2 bg-surface border border-border rounded-md text-sm focus:ring-1 focus:ring-primary focus:border-primary font-sans text-on-surface appearance-none"
            >
      ...
      <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4 md:gap-6">
        {filteredRoasts.length > 0 ? (
            ...
            >
              ...
              <div className="flex-1 p-6 pt-4">
                <div className="flex-1 mb-6">
                  ...
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded text-xs font-semibold bg-surface-variant text-on-surface-variant font-sans">
                    {roast.type}
                  </span>
                </div>
          ...
          <div className="col-span-full py-12 text-center text-on-surface-variant font-sans bg-surface rounded-lg border border-dashed border-border">
            No roasts found matching your search and filter.
          </div>
        )}
      </div>
    </div>
